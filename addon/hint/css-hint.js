define(['lib/codemirror/lib/codemirror', 'underscore'], function (CodeMirror, _) {
    ...
    // keywrods basically from CodeMirror css mode and some added
    var atMediaTypes = [
        ...
        if (str.substring(0, sub.length).toLowerCase() === sub) {
            return (function (str, sub) {
                return function (elem /*, self, data*/) {
                    $(elem).html('')
                    .append($('<span class="CodeMirror-hint-common-string">').text(sub))
                    .append($('<span>').text(str.substring(sub.length)));
                };
            })(str, sub);
        }
    }
    ...
    CodeMirror.registerHelper('hint', 'css', function (editor, options) {
        var cursor = editor.getCursor();
        var token = editor.getTokenAt(cursor, true);

        if (token.start <= cursor.ch && cursor.ch <= token.end) {
            var tstr = token.string.toLowerCase();
            var list = _.sortBy(_.compact(_.map(allWords, function (w) {
        ...
        return {list: allWords, from: cursor, to: cursor};
    });
});
