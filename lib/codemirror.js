...
(function(mod) {
  ...
  function readInput(cm) {
    ...
    for (var i = doc.sel.ranges.length - 1; i >= 0; i--) {
      ...
      if (inserted && !cm.state.pasteIncoming && cm.options.electricChars &&
        ...
        if (mode.electricChars && (!mode.checkElectric || mode.checkElectric(cm))) {
  ...
  function onContextMenu(cm, e) {
    return; // Do nithing - by hina.
            // See issue WTC-428 and https://github.com/marijnh/CodeMirror/issues/2704
  }
  ...
  function calculateScrollPos(cm, x1, y1, x2, y2) {
    /* too complicated and causes an infinite loop in the above scrollPosIntoView
       when screen or screenw is too small.
     ...
     */
    var display = cm.display, result = {};
    if (y1 < 0) y1 = 0;
    if (x1 < 0) x1 = 0;

    var screenh = display.scroller.clientHeight - scrollerCutOff,
        screentop = display.scroller.scrollTop;
    if (y2 < screentop || y1 > screentop + screenh) {
        result.scrollTop = y1;
    }
    var screenw = display.scroller.clientWidth - scrollerCutOff,
        screenleft = display.scroller.scrollLeft;
    if (x2 < screenleft || x1 > screenleft + screenw) {
        result.scrollLeft = x1;
    }
  ...
  function indentLine(cm, n, how, aggressive) {
    ...
    if (indentString != curSpaceString) {
      replaceRange(cm.doc, indentString, Pos(n, 0), Pos(n, curSpaceString.length), "+auto");
    } else {
  ...
  keyMap.pcDefault = {
    ...
    "Ctrl-Home": "goDocStart", "Ctrl-Up": "scrollUp", "Ctrl-End": "goDocEnd", "Ctrl-Down": "scrollDown",
